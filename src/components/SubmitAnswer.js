...
function SubmitAnswer({ question, dispatch, authedUser }) {
  if (question) {
    ...
    return (
      <form className='a-container' onSubmit={(e) => handleSubmit(e, dispatch, authedUser, question.id)}>
        <span className='a-header'>Would You Rather?</span>
        <div>
          <input type='radio' id='option-one' name='answer' value='optionOne' defaultChecked />
